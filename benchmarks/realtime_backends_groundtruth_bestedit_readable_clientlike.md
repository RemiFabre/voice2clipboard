...
- `benchmarks/realtime_backends_groundtruth_bestedit_voxmlx_clientlike.json`
...
| faster:whisper-medium | 5 | 0.006 | 0.006 | 13.098 | 13.098 | 0.740 | excellent |
| faster:whisper-small | 5 | 0.068 | 0.060 | 4.774 | 4.774 | 0.279 | good |
| voxmlx:mistralai/Voxtral-Mini-4B-Realtime-2602|mode=clientlike|chunk=128|commit=0.7|idle=1.5 | 5 | 0.067 | 0.047 | 34.927 | 2.816 | 1.193 | good |
...
- Config: `faster:whisper-medium`
  WER=0.000 CER=0.000 | total=9.09s first=9.09s | edits(sub/ins/del)=0/0/0 (distance=0)
  Hypothesis:
  Hey, just testing if this works, hopefully it does.
...
- Config: `voxmlx:mistralai/Voxtral-Mini-4B-Realtime-2602|mode=clientlike|chunk=128|commit=0.7|idle=1.5`
  WER=0.000 CER=0.023 | total=6.54s first=2.09s | edits(sub/ins/del)=1/0/0 (distance=1)
  Hypothesis:
  Hey, just testing if this works. Hopefully it does.
...
- Config: `faster:whisper-medium`
  WER=0.000 CER=0.004 | total=9.87s first=9.87s | edits(sub/ins/del)=1/0/0 (distance=1)
  Hypothesis:
  Ok, testing if this transcript works and if it's fast enough. There is some noise around, so let's see if the transcription is good. My sentences should be normal, so if you see weird words or stuff like that, that shouldn't make sense. It's probably transcription mistakes, but that's another problem.
...
- Config: `voxmlx:mistralai/Voxtral-Mini-4B-Realtime-2602|mode=clientlike|chunk=128|commit=0.7|idle=1.5`
  WER=0.059 CER=0.012 | total=24.29s first=2.72s | edits(sub/ins/del)=2/0/1 (distance=3)
  Hypothesis:
  Okay, testing if this transcript works and if it's fast enough. There's some noise around, so let's see if the transcription is good. My sentences should be normal, so if you see weird words or stuff like that, that shouldn't make sense, it's probably transcription mistakes, but that's another problem.
...
- Config: `faster:whisper-medium`
  WER=0.000 CER=0.000 | total=9.88s first=9.88s | edits(sub/ins/del)=0/0/0 (distance=0)
  Hypothesis:
  Okay, I'm trying again. Let's see if this time the transcription is correct. I'm doing a bit of a long message. Let's assume this is a message of like, I don't know, maybe 15 seconds. This causes inverse kinematics in robotics in a very nosy environment. Hopefully this works properly.
...
- Config: `voxmlx:mistralai/Voxtral-Mini-4B-Realtime-2602|mode=clientlike|chunk=128|commit=0.7|idle=1.5`
  WER=0.020 CER=0.004 | total=20.55s first=2.55s | edits(sub/ins/del)=1/0/0 (distance=1)
  Hypothesis:
  Okay, I'm trying again. Let's see if this time the transcription is correct. I'm doing a bit of a long message. Let's assume this is a message of like, I don't know, maybe 15 seconds. This causes inverse kinematics in robotics in a very noisy environment. Hopefully this works properly.
...
- Config: `faster:whisper-medium`
  WER=0.030 CER=0.025 | total=18.62s first=18.62s | edits(sub/ins/del)=0/0/3 (distance=3)
  Hypothesis:
...
- Config: `voxmlx:mistralai/Voxtral-Mini-4B-Realtime-2602|mode=clientlike|chunk=128|commit=0.7|idle=1.5`
  WER=0.160 CER=0.130 | total=71.90s first=3.35s | edits(sub/ins/del)=8/12/3 (distance=23)
  Hypothesis:
  Can we first do a bit more research on this? Like, for example, those MLX, allow for streaming or do we need specifically whisper streaming for it to work? And we're only talking about Whisper. Are there other options in currently? I mean, this seems to be very rapidly moving subject, I guess. Every week I hear about a new model that does I want you to research recent models that be better than than whisper for us maybe that's not the case because you need to implement something specific for for Apple for it to be for it to be better but I need this research phase first.
...
- Config: `faster:whisper-medium`
  WER=0.000 CER=0.000 | total=18.03s first=18.03s | edits(sub/ins/del)=0/0/0 (distance=0)
  Hypothesis:
  Ok, donc je refais un message vocal en français. J'aimerais qu'on utilise celui-ci pour le benchmark. Je vais faire exprès d'utiliser des mots, des fois avec des longues pauses. Je me demande si on peut poser des questions aussi. Là, l'environnement est très silencieux. Des fois, il y a des petits bruits de chaise, des trucs comme ça, mais grosso modo là, c'est un contexte idéal. Je pourrais peut-être travailler comme ça. Et oui, je me demande si je peux utiliser du franglais, des mots un peu archotiques. Tu vois, si je dis qu'est-ce que c'est que ce bordel de merde, est-ce que c'est bien traduit ? Si je dis à la future, elle n'a pas été débatant, c'est de ma faute. Tu vois, est-ce que ce genre de phrase fonctionne ?
...
- Config: `voxmlx:mistralai/Voxtral-Mini-4B-Realtime-2602|mode=clientlike|chunk=128|commit=0.7|idle=1.5`
  WER=0.096 CER=0.066 | total=51.35s first=3.36s | edits(sub/ins/del)=10/5/5 (distance=20)
  Hypothesis:
  Ok, donc je refais un message vocal en français. J'aimerais qu'on utilise celui-ci pour le benchmark. Je vais faire exprès d'utiliser des mots des fois avec des longues pauses, Je pense que si, je me demande si on peut poser des questions aussi. Là, l'environnement est très silencieux. Bon, des fois, il y a des petits bruits de chaises, des trucs comme ça, mais grosso modo, là, c'est un contexte idéal. Je pourrais peut-être travailler comme ça. Et ouais, je me demande si je peux utiliser du franglais, des mots un peu argotiques, Tu vois, si je dis qu'est-ce que c'est que ce bordel de merde, est-ce que c'est bien traduit? Si je dis la feature, elle n'a pas été débattante. C'est de ma faute. Est-ce que ce genre de phrase fonctionne?
